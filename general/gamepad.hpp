...
class GamePad {
 ...
 public:
  ...
  inline bool GetButton(ButtonNames name) const { return GetButton((int)name); }

  inline int GetAxisRaw(int id) const { return id < axis_num ? axises[id] : 0; }
  inline int GetAxisRaw(AxisNames name) const { return GetAxisRaw((int)name); }
